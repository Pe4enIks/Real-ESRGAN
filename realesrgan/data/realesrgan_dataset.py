...
class RealESRGANDataset(data.Dataset):
    ...
    It also generates blur kernels and sinc kernels
    for generating low-quality images.
    Note that the low-quality images are processed
    in tensors on GPUS for faster processing.
    ...
    Args:
        opt (dict): Config for train datasets. It contains the following keys:
            ...
            use_rot (bool): Use rotation
                (use vertical flip and transposing h and w for implementation).
    ...
    def __init__(self, opt):
        ...
        if self.io_backend_opt['type'] == 'lmdb':
            ...
            if not self.gt_folder.endswith('.lmdb'):
                raise ValueError(
                    f"'dataroot_gt' should end with '.lmdb', "
                    "but received {self.gt_folder}"
                )
            with open(osp.join(self.gt_folder, 'meta_info.txt')) as fin:
                self.paths = [line.split('.')[0] for line in fin]
        else:
            # disk backend with meta_info
            # Each line in the meta_info describes the relative path to an img
    ...
    def __getitem__(self, index):
        ...
        while retry > 0:
            try:
                ...
                logger.warning(
                    f'File client error: {e}, '
                    'remaining retry times: {retry - 1}'
                )
        ...
        if img_gt.shape[0] > crop_pad_size or img_gt.shape[1] > crop_pad_size:
            ...
            img_gt = \
                img_gt[top:top + crop_pad_size, left:left + crop_pad_size, ...]
